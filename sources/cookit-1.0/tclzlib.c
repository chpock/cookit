...
static int
Zlib_Cmd(ClientData dummy, Tcl_Interp *ip, int objc, Tcl_Obj *CONST objv[])
{
  ...
  static CONST char* cmds[] = {
    "adler32", "crc32", "compress", "deflate", "decompress", "inflate", "sdecompress", "sinflate", NULL,
  };
...
Tclzlib_Init(Tcl_Interp *interp)
{
    ...
    if (Tcl_InitStubs(interp, "8.5", 0) == NULL) {
     ...
     if (Tcl_PkgProvide(interp, "zlib", "0.11") == TCL_ERROR) {
         return TCL_ERROR;
     }
     Tcl_CreateObjCommand(interp, "zlib", Zlib_Cmd, NULL, NULL);
     return TCL_OK;
}
