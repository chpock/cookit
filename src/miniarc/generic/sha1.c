...
static Tcl_ObjType sha1_type; /* fast internal access representation */
...
static int
sha1_from_any(Tcl_Interp* interp, Tcl_Obj* obj)
{
    assert(0);
    return TCL_ERROR;
}
...
Tcl_ObjType sha1_type = {
    "sha1c", sha1_free_rep, sha1_dup_rep, sha1_string_rep, sha1_from_any
};
    ...
    Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 3) {
        if (objv[2]->typePtr != &sha1_type 
            && sha1_from_any(interp, objv[2]) != TCL_OK) return TCL_ERROR;
        obj = objv[2];
        if (Tcl_IsShared(obj)) {
...
int
Sha1_Init(Tcl_Interp *interp)
{
    Tcl_CreateObjCommand(interp, "sha1", (Tcl_ObjCmdProc *) Sha1ObjCmd,
